    ...
    const fetchMaterias = async () => {
      try {
        const res = await axios.get(
          `https://proyectofinal-backend-1-uqej.onrender.com/users/${userId}/profesor`
        );
          ...
          materiasBase.map(async (materia) => {
            try {
              const notaRes = await axios.get(
                `https://proyectofinal-backend-1-uqej.onrender.com/materia/${materia.id}/${userId}`
              );
              const nota = notaRes.data.nota ?? null;
              return { ...materia, nota };
            } catch (error) {
              console.error(
                `Error al obtener nota de materia ${materia.name}:`,
                error
              );
              showToast(
                `❌ Error al obtener nota de materia ${materia.name}`,
